...
correction does not enlarge the existing model, tool, or recursion budgets and
does not promote chat text or fallback content into the canonical artifact. If
the one correction still produces no valid file, finalization keeps the
existing fallback and fail-closed delivery behavior.
