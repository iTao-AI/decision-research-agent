...
Review baseline: `d99e9db6857411b0d44ec51a8fbd77d29a124c18`. These refinements complete the same scope; they do not authorize backend or dependency changes.

1. **Default route and small screens.** Test the ordinary `/` route as well as the showcase query URLs. In Static Demo, the operator screen menu, technical content, Live controls and authority details must start collapsed. On small screens, the tall stage rail must be a compact navigation or a collapsed disclosure. At a 390 x 844 viewport, the initial question and recommendation must both be visible without scrolling through technical navigation. Preserve access to all operator screens and make Live controls visible when the user explicitly selects Live mode.
2. **One reading path.** In the static case, show the question/recommendation, comparison and linked findings first. Place the complete `ResultReader` below them in a native disclosure labeled `完整报告与下载`, initially closed. Live results keep their primary reader expanded. This removes the repeated question, recommendation, comparison and findings from the default scroll path without changing any artifact bytes. Move full hashes and UTF-8/internal metadata into the reader's technical disclosure. Use a fixture-specific hash label for Static Demo; a bundled fixture was not returned by a live service.
3. **Sources without false warnings.** Selecting a source directly or loading the initial page must resolve that source's actual associated claims even without an incoming claimId. Show the matching claims/excerpts, or a neutral statement when the source has no claim; do not report that a known reference is missing. Preserve the explicit return-to-referring-claim action. Do not pair a missing selected source with the first unrelated evidence record's metadata. In Live mode, when the current evidence collection/run changes, keep selection valid or select a source from the current collection; never leave a stale selected ID with an unexplained empty detail pane.
4. **Correct the first claim.** Keep `claim_bounded_assistant` linked to `ev_pilot_scope`, but use `试点先覆盖政策检索与答复草稿，发送前仍需客服审核。` with the exact excerpt `第一阶段帮助客服人员定位政策并起草答案。答案由客服人员审核后发送。`. The old statement claimed existing read access while citing a document that described requirements, not system access. Update its occurrence in the report and recompute the affected artifact hash. The system-access document continues to support the option comparison and refund-write claim. Exact substring/hash tests cannot replace this semantic check.
5. **Reading and download verification.** Preserve line breaks when falling back to unsupported Markdown, particularly the tables emitted by the existing report renderer; do not collapse table rows into a space-joined paragraph. Keep the reader small and safe, with no new Markdown dependency. Verify an actual user-triggered download in an available real browser and compare the saved UTF-8 bytes with the displayed artifact. An in-app browser download wait timed out without a page error during review; this is an unresolved observation, not a proven cause. If an existing standalone browser succeeds, record that limitation rather than changing code speculatively. If the download is defective, repair its lifecycle with a regression test. Do not equate a mocked anchor click with completed browser download.
6. **README opening.** Put the product result and entry point before the framework/terminology list. Use this Chinese opening: `Decision Research Agent 把开放研究问题整理成带来源依据的研究报告。调用方可以通过 CLI 或 API 发起任务、观察进度，并取回后端确认可交付的报告。` Follow with the concrete support-pilot example, source inspection and report download; then explain Static Demo versus Live Backend once. Move the framework terminology into an engineering section below the product explanation. Provide equivalent English copy, preserve accurate release information, and do not add enterprise-adoption claims.

Add targeted checks for initial source selection, the invalidated Live selection, default-route disclosure state, and meaningful download/reader behavior. After these changes converge, run the affected frontend suite, lint/build and relevant documentation contracts once, then refresh the existing screenshot set with valid provenance. Review both `/` and the query routes at desktop and mobile widths; a successful screenshot of `/?showcase=overview` alone does not verify `/`.
