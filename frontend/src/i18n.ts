...
export const copy = {
  zh: {
    ...
    live: {
      ...
      knownRun: "已知 run_id",
      knownRunHint:
        "此输入仅存在当前浏览器会话；重新观察只发 GET，不会创建新运行。格式：首字符为 ASCII 字母或数字，其余仅可为 ASCII 字母、数字、点、下划线或连字符，最长 128 个字符。",
      queryBytes: (current: number) => `UTF-8 bytes: ${current} / 4096`,
      queryBlank: "请先输入研究问题。",
      queryTooLarge: "请将研究问题缩短到 4096 UTF-8 bytes 以内。",
      knownRunBlank: "请输入已知 run_id。",
      knownRunTooLong: "run_id 不能超过 128 个字符。",
      knownRunInvalidFormat:
        "run_id 必须以字母或数字开头，只能包含 ASCII 字母、数字、点、下划线或连字符；最长 128 个字符。",
      checkHealth: "检查后端",
      runResult: "运行并获取结果",
      observeKnownRun: "观察已知运行",
    ...
    reader: {
      ...
      download: "下载报告",
      fullReportLabel: "完整报告与下载",
      metadataLabel: "报告元数据",
      fixtureHash: "本地 fixture 的 content_hash",
      serviceReportedHash: "服务报告的 content_hash",
      exactBytes: "下载将保留原始 UTF-8 字节",
      viewLabel: "报告视图",
      readView: "阅读",
      rawView: "原文",
      technicalDetails: "运行详情",
      unavailableTitle: "报告暂不可用",
      unavailableDetail: "当前状态没有可交付的 canonical result，因此没有下载操作。"
    },
    showcase: {
      railLabel: "研究路径",
      ...
      headerTitle: "把研究结论和依据放在一起",
      headerDescription: "先读决策简报，再打开每条结论对应的本地合成资料。",
      normalBadge: "静态案例 · 已交付",
      blockedBadge: "需要复核",
      brief: {
        reportLabel: "研究报告 / Decision Brief",
        recommendationLabel: "研究结论",
        recommendationDetail:
          "先试点内部知识助手，让客服人员检索已批准的政策材料、生成带依据的草稿，再由授权工作人员审核后发送。",
        criteriaLabel: "比较依据",
        optionLabel: "方案",
        integrationLabel: "当前接入",
        humanDecisionLabel: "人工决策",
        evaluationLabel: "试点可评估",
        findingsLabel: "支持性发现",
        claimLabel: "结论",
        inspectEvidence: "查看依据",
        evidenceLabel: "Evidence",
        staticDisclosure:
          "本页是本地合成案例，不代表真实客户试点、模型评测或生产收益。节省时间与采用率仍待测量。",
        sourceListLabel: "依据来源",
        sourceListDetail: "选择一条来源，查看它支持的具体内容。",
        sourceDetailLabel: "选中的依据",
        sourceTextLabel: "本地 fixture 正文",
        excerptLabel: "对应摘录",
        sourceObserved: "本次案例已纳入",
        sourceUnconfirmed: "本次运行待确认",
        sourceUnavailable: "本次运行未观察到",
        noExcerpt: "当前运行没有观察到可展示的 excerpt；不会从报告正文推断。",
        returnToClaim: "返回对应结论",
        sourceUrl: "source_url",
        sourceUrlBlocked: "该 URL 未通过 HTTP(S) 安全检查",
        noClaim: "该来源目前没有关联的 claim。",
        claimsUnavailable: "必要的政策资料访问尚未确认，以下结论不能作为本次运行已验证的依据。",
        nextClarification: "下一步需要澄清",
        nextClarificationDetail:
          "确认已批准政策材料和退款处理规则的访问权限，再重新评估是否能扩大试点范围。"
      },
      judgmentLabel: "判断侧栏",
      ...
      overview: {
        ...
        evidence: "3 条 Evidence 已冻结，claim 与 source 可回溯。",
  ...
  en: {
    ...
    live: {
      ...
      knownRun: "Known run_id",
      knownRunHint:
        "This input is browser-session-only; reattachment uses GET only and creates no new run. Exact format: start with an ASCII letter or digit, then use only ASCII letters, digits, dot, underscore, or hyphen; 128 characters max.",
      queryBytes: (current: number) => `UTF-8 bytes: ${current} / 4096`,
      ...
      knownRunBlank: "Enter a known run_id.",
      knownRunTooLong: "run_id cannot exceed 128 characters.",
      knownRunInvalidFormat:
        "run_id must start with a letter or digit and contain only ASCII letters, digits, dot, underscore, or hyphen; 128 characters max.",
      checkHealth: "Check backend",
      runResult: "Run and fetch result",
      observeKnownRun: "Observe known run",
    ...
    reader: {
      ...
      documentLabel: "Decision brief",
      staticMode: "Static case",
      liveMode: "Live Backend",
      download: "Download report",
      fullReportLabel: "Full report and download",
      metadataLabel: "Report metadata",
      fixtureHash: "Local fixture content_hash",
      serviceReportedHash: "Service-reported content_hash",
      exactBytes: "Download preserves the original UTF-8 bytes",
      viewLabel: "Report view",
      readView: "Read",
      rawView: "Raw",
      technicalDetails: "Run details",
      unavailableTitle: "Report unavailable",
      unavailableDetail: "The current state has no deliverable canonical result, so no download is offered."
    },
    showcase: {
      ...
      question: "客服团队应该先试点内部知识助手，还是直接让 Agent 自动处理退款？",
      headerTitle: "Put the research conclusion beside its evidence",
      headerDescription: "Read the decision brief first, then inspect the local synthetic material behind each claim.",
      normalBadge: "Static case · delivered",
      blockedBadge: "Review required",
      brief: {
        reportLabel: "Research report / Decision brief",
        recommendationLabel: "Research conclusion",
        recommendationDetail:
          "Start with an internal knowledge assistant: support staff retrieve approved policy material, draft an evidence-bound answer, and an authorized staff member reviews it before sending.",
        criteriaLabel: "Comparison criteria",
        optionLabel: "Option",
        integrationLabel: "Current integration",
        humanDecisionLabel: "Human decision",
        evaluationLabel: "What the pilot evaluates",
        findingsLabel: "Supporting findings",
        claimLabel: "Claim",
        inspectEvidence: "Inspect evidence",
        evidenceLabel: "Evidence",
        staticDisclosure:
          "This is a local synthetic case, not a real customer trial, model evaluation, or production impact claim. Time savings and adoption remain to be measured.",
        sourceListLabel: "Supporting sources",
        sourceListDetail: "Select a source to inspect the exact material behind it.",
        sourceDetailLabel: "Selected evidence",
        sourceTextLabel: "Local fixture text",
        excerptLabel: "Matching excerpt",
        sourceObserved: "Included in this case",
        sourceUnconfirmed: "Unconfirmed for this run",
        sourceUnavailable: "Not observed in this run",
        noExcerpt: "No excerpt was observed for this run; the UI does not infer one from the report.",
        returnToClaim: "Return to referring claim",
        sourceUrl: "source_url",
        sourceUrlBlocked: "This URL did not pass the HTTP(S) safety check",
        noClaim: "This source has no associated claim.",
        claimsUnavailable: "Access to the needed policy material is unconfirmed, so these claims are not presented as verified evidence for this run.",
        nextClarification: "Clarify next",
        nextClarificationDetail:
          "Confirm access to approved policy material and the refund handling rules before reassessing a broader pilot."
      },
      ...
      overview: {
        ...
        evidenceTitle: "Evidence",
        evidence: "Three Evidence entries are frozen and traceable to claims.",
        reviewTitle: "Judgment",
      ...
      blocked: {
        ...
        reportUnavailable: "Report unavailable; download is disabled",
